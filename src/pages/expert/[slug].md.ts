import type { APIRoute } from 'astro';
import { getExperts, getExpert } from '@/data/bundles';
import { getSkill } from '@/data/skills';

export const prerender = true;

export function getStaticPaths() {
  return getExperts().map(e => ({ params: { slug: e.slug } }));
}

export const GET: APIRoute = ({ params }) => {
  const expert = getExpert(params.slug!);
  if (!expert) {
    return new Response('Not found', { status: 404 });
  }

  const lines: string[] = [];
  const url = `https://clawhub.md/expert/${expert.slug}.md`;

  // Header
  lines.push(`# ${expert.name}`);
  lines.push('');
  lines.push(`> **Agentic setup file** — share this URL with your agent and it will set everything up for you:`);
  lines.push(`> \`${url}\``);
  lines.push('');
  lines.push(`**Goal:** ${expert.goal}`);
  lines.push('');
  lines.push(`**What you\'ll have:** ${expert.outcome}`);
  lines.push('');
  lines.push('---');
  lines.push('');

  // Step 1: Install
  lines.push('## Step 1: Install');
  lines.push('');
  const installCmd = expert.installAll
    ?? expert.skillsWithReason.map(s => `clawhub install ${s.slug}`).join('\n');
  lines.push('```bash');
  lines.push(installCmd);
  lines.push('```');
  lines.push('');

  // Step 2: Configure (per skill)
  const skillsNeedingSetup = expert.skillsWithReason
    .map(entry => ({
      entry,
      skill: getSkill(
        entry.slug.split('/')[0]!,
        entry.slug.split('/')[1]!,
      ),
    }))
    .filter(({ skill }) => skill?.setupSteps && skill.setupSteps.length > 0);

  if (skillsNeedingSetup.length > 0) {
    lines.push('## Step 2: Configure');
    lines.push('');
    lines.push('Each skill may need credentials or auth before it can act on your behalf.');
    lines.push('');

    for (const { entry, skill } of skillsNeedingSetup) {
      lines.push(`### ${entry.slug}`);
      lines.push('');
      lines.push(`_${entry.reason}_`);
      lines.push('');
      for (const step of skill!.setupSteps!) {
        lines.push(`- ${step}`);
      }
      lines.push('');
    }
  }

  // Step 3 (or 2): Use
  const useStep = skillsNeedingSetup.length > 0 ? '3' : '2';
  lines.push(`## Step ${useStep}: Try it`);
  lines.push('');
  lines.push('After setup, say these to your agent to verify everything works:');
  lines.push('');

  for (const entry of expert.skillsWithReason) {
    if (entry.examples && entry.examples.length > 0) {
      lines.push(`**${entry.slug}**`);
      lines.push('');
      for (const ex of entry.examples.slice(0, 3)) {
        lines.push(`- "${ex}"`);
      }
      lines.push('');
    }
  }

  lines.push('---');
  lines.push('');
  lines.push(`*${expert.name} · [clawhub.md/expert/${expert.slug}](https://clawhub.md/expert/${expert.slug})*`);

  return new Response(lines.join('\n'), {
    headers: {
      'Content-Type': 'text/markdown; charset=utf-8',
      'Cache-Control': 'public, max-age=3600',
    },
  });
};
